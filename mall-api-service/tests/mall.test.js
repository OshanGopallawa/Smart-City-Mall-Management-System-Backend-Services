...
jest.mock('mongoose', () => ({
  ...
  connection: { readyState: 1, name: 'mall_api_db', on: jest.fn() },
  Schema: class { constructor() { this.methods = {}; this.index = jest.fn(); } },
  model: jest.fn(() => ({
    find: jest.fn().mockReturnValue({ skip: jest.fn().mockReturnValue({ limit: jest.fn().mockReturnValue({ sort: jest.fn().mockResolvedValue([]) }) }) }),
...
describe('Mall API Service', () => {
  ...
  it('POST /api/stores requires auth', async () => {
    const res = await request(app).post('/api/stores').send({ name: 'Test', category: 'Fashion', floor: '1', unit_number: 'A-1' });
